...
if(isset($_GET['id'])){
    ...
    $sql = "SELECT b.total_tickets,s.cost,s.handicap_seats,s.id AS s_id
    FROM bookings b
...
if(isset($_POST['submit'])){
    ...
    if($stmt1->execute()){
        ...
        //seats records
        $seats = $row['handicap_seats'] - $row['total_tickets'];

        //update available seats
        $stmt2=$connection->prepare("UPDATE schedules SET handicap_seats = ? WHERE id = ?");
            ...
            <div class="form-wrap">
                <form action="" method="POST">
                    ...
                    <div class="input-grid">
                        ...
                        <input type="email" placeholder="Email" name="email" value="<?php echo ($row['cost']*$row['total_tickets'])*0.8 ?>" readonly>
                    </div>
                    <div class="form-btn">
